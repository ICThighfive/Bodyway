...
<h1 align="center"><%= diary.getDiaryNo() %>번 운동일지 수정 페이지</h1>
<br>
<!-- form 에서 입력값들과 파일을 함께 전송하려면 
반드시 속성을 추가해야 함 : enctype="multipart/form-data" -->
<form action="/semi/dupdate" method="post" enctype="multipart/form-data">
<input type="hidden" name="no" value="<%= diary.getDiaryNo() %>">
<input type="hidden" name="ofile" value="<%= diary.getDiaryOriginalImage() %>">
<input type="hidden" name="rfile" value="<%= diary.getDiaryRenameImage() %>">
...
<div class="col text-center">
	...
	<input type="button" class="btn btn-outline-primary btn-sm" value="목록" onclick="javascript:location.href='/semi/dlist'; return false;"> &nbsp;
</div>
</th></tr>
</table>
</form>
